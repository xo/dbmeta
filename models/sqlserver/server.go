...
func registerForeign() {
	...
	dbmeta.ForeignTables.Register(dbmeta.SQLServer, &dbmeta.Binding[dbmeta.ForeignTable]{
		Stmt: dbmeta.Stmt{
			...
			{{Min: v13, Query: `, t.name AS "name"`}},
			{{Min: v13, Query: `, COALESCE(ds.name, '') AS "server"`}},
			{{Min: v13, Query: `, NULLIF(t.location, '') AS "options"`}},
			{{Min: v13, Query: `, ` + commentOn("t.object_id") + ` AS "comment"`}},
			{{Min: v13, Query: `FROM sys.external_tables t`}},
			{{Min: v13, Query: `JOIN sys.schemas s ON s.schema_id = t.schema_id`}},
			{{Min: v13, Query: `LEFT JOIN sys.external_data_sources ds` +
				` ON ds.data_source_id = t.data_source_id`}},
			{{Min: v13, Query: `WHERE ` + notSystem}},
			{{Min: v13, Query: `AND (@schema = '' OR s.name LIKE @schema)`}},
			{{Min: v13, Query: `AND (@name = '' OR t.name LIKE @name)`}},
			{{Min: v13, Query: `ORDER BY 1, 2`}},
		},
		Fields: []dbmeta.Field{
...
func registerStats() {
	dbmeta.ColumnStats.Register(dbmeta.SQLServer, &dbmeta.Binding[dbmeta.ColumnStat]{
		Stmt: dbmeta.Stmt{
			{{Min: v11, Query: `SELECT DB_NAME() AS "catalog"`}},
			{{Min: v11, Query: `, s.name AS "schema"`}},
			{{Min: v11, Query: `, o.name AS "table"`}},
			{{Min: v11, Query: `, c.name AS "name"`}},
			{{Min: v11, Query: `, NULL AS "avg_width"`}},
			{{Min: v11, Query: `, NULL AS "null_frac"`}},
			{{Min: v11, Query: `, CAST(sp.rows AS float) AS "distinct"`}},
			{{Min: v11, Query: `, NULL AS "min"`}},
			{{Min: v11, Query: `, NULL AS "max"`}},
			{{Min: v11, Query: `, NULL AS "mean"`}},
			{{Min: v11, Query: `, NULL AS "top_n"`}},
			{{Min: v11, Query: `, NULL AS "top_n_freqs"`}},
			{{Min: v11, Query: `FROM sys.stats st`}},
			{{Min: v11, Query: `CROSS APPLY sys.dm_db_stats_properties(st.object_id, st.stats_id) sp`}},
			{{Min: v11, Query: `JOIN sys.stats_columns sc` +
				` ON sc.object_id = st.object_id AND sc.stats_id = st.stats_id`}},
			{{Min: v11, Query: `JOIN sys.objects o ON o.object_id = st.object_id`}},
			{{Min: v11, Query: `JOIN sys.schemas s ON s.schema_id = o.schema_id`}},
			{{Min: v11, Query: `JOIN sys.columns c` +
			...
			{{Min: v11, Query: `AND (@parent = '' OR o.name LIKE @parent)`}},
			{{Min: v11, Query: `AND (@name = '' OR c.name LIKE @name)`}},
			{{Min: v11, Query: `ORDER BY 2, 3, 4`}},
		},
		Fields: []dbmeta.Field{
