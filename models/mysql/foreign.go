...
func registerAggregates() {
	dbmeta.Aggregates.Register(dbmeta.MySQL, &dbmeta.Binding[dbmeta.Function]{
		Stmt: dbmeta.Stmt{
			{{SQL: `SELECT 'def' AS "catalog"`}},
			{{SQL: `, p.db AS "schema"`}},
			{{SQL: `, p.name AS "name"`}},
			{{SQL: `, 'agg' AS "kind"`}},
			{{SQL: `, CONVERT(p.returns USING utf8mb4) AS "result_type"`}},
			{{SQL: `, CONVERT(p.param_list USING utf8mb4) AS "arg_types"`}},
			{{SQL: `, LOWER(p.is_deterministic) AS "volatility"`}},
			{{SQL: `, '' AS "parallel"`}},
			{{SQL: `, p.definer AS "owner"`}},
			{{SQL: `, LOWER(p.security_type) AS "security"`}},
			{{SQL: `, NULL AS "access"`}},
			{{SQL: `, 'sql' AS "language"`}},
			{{SQL: `, CONVERT(p.body USING utf8mb4) AS "source"`}},
			{{SQL: `, NULLIF(CONVERT(p.comment USING utf8mb4), '') AS "comment"`}},
			{{SQL: `FROM mysql.proc p`}},
			{{SQL: `WHERE p.aggregate = 'GROUP'`}},
			{{SQL: `AND (@with_system OR p.db NOT IN (` + systemSchemas + `))`}},
			{{SQL: `AND (@schema = '' OR p.db LIKE @schema)`}},
			{{SQL: `AND (@name = '' OR p.name LIKE @name)`}},
			// a compiled aggregate is global, so it has no schema to filter on
			// and it appears whatever @schema asks for
			{{SQL: `UNION ALL`}},
			{{SQL: `SELECT 'def', '', f.name, 'agg'`}},
			{{SQL: `, CASE f.ret WHEN 0 THEN 'string' WHEN 1 THEN 'real' WHEN 2 THEN 'int'` +
				` WHEN 3 THEN 'row' WHEN 4 THEN 'decimal' ELSE CAST(f.ret AS CHAR) END`}},
			{{SQL: `, NULL, '', '', '', '', NULL, 'c', f.dl, NULL`}},
			{{SQL: `FROM mysql.func f`}},
			{{SQL: `WHERE f.type = 'aggregate'`}},
			{{SQL: `AND (@name = '' OR f.name LIKE @name)`}},
			{{SQL: `ORDER BY 2, 3`}},
		},
		Fields: []dbmeta.Field{
