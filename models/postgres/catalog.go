...
func registerDatabases() {
	dbmeta.Databases.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Database]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.array_to_string(d.datacl, E'\n'), '') AS "access"`}},
			{{SQL: `, COALESCE(t.spcname, '') AS "tablespace"`}},
			{{SQL: `, CASE WHEN pg_catalog.has_database_privilege(d.datname, 'CONNECT')` +
				` THEN pg_catalog.pg_size_pretty(pg_catalog.pg_database_size(d.datname))` +
				` ELSE 'no access' END AS "size"`}},
			{{SQL: `, COALESCE(pg_catalog.shobj_description(d.oid, 'pg_database'), '') AS "comment"`}},
...
func registerTablespaces() {
	dbmeta.Tablespaces.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Tablespace]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.array_to_string(spcoptions, ', '), '') AS "options"`}},
			{{SQL: `, pg_catalog.pg_size_pretty(pg_catalog.pg_tablespace_size(oid)) AS "size"`}},
			{{SQL: `, COALESCE(pg_catalog.array_to_string(spcacl, E'\n'), '') AS "access"`}},
			{{SQL: `, COALESCE(pg_catalog.shobj_description(oid, 'pg_tablespace'), '') AS "comment"`}},
...
func registerAccessMethods() {
	dbmeta.AccessMethods.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.AccessMethod]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.obj_description(oid, 'pg_am'), '') AS "comment"`}},
...
func registerLanguages() {
	dbmeta.Languages.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Language]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.array_to_string(l.lanacl, E'\n'), '') AS "access"`}},
			{{SQL: `, COALESCE(d.description, '') AS "comment"`}},
...
func registerCasts() {
	dbmeta.Casts.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Cast]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(p.proleakproof, false) AS "leakproof"`}},
			{{SQL: `, COALESCE(d.description, '') AS "comment"`}},
...
func registerCollations() {
	dbmeta.Collations.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Collation]{
		Stmt: dbmeta.Stmt{
			...
			{
				{SQL: `, '' AS "locale"`},
				{Min: v12, SQL: `, COALESCE(c.colliculocale, '') AS "locale"`},
				{Min: v17, SQL: `, COALESCE(c.colllocale, '') AS "locale"`},
			},
			{
				{SQL: `, true AS "deterministic"`},
				{Min: v12, SQL: `, c.collisdeterministic AS "deterministic"`},
			},
			{{SQL: `, COALESCE(pg_catalog.obj_description(c.oid, 'pg_collation'), '') AS "comment"`}},
...
func registerLargeObjects() {
	dbmeta.LargeObjects.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.LargeObject]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.array_to_string(o.lomacl, E'\n'), '') AS "access"`}},
			{{SQL: `, COALESCE(pg_catalog.obj_description(o.oid, 'pg_largeobject'), '') AS "comment"`}},
...
func registerEventTriggers() {
	dbmeta.EventTriggers.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.EventTrigger]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(pg_catalog.array_to_string(e.evttags, ', '), '') AS "tags"`}},
			{{SQL: `, COALESCE(pg_catalog.obj_description(e.oid, 'pg_event_trigger'), '') AS "comment"`}},
...
func registerSettings() {
	dbmeta.Settings.Register(dbmeta.PostgreSQL, &dbmeta.Binding[dbmeta.Setting]{
		Stmt: dbmeta.Stmt{
			...
			{{SQL: `, COALESCE(s.setting, '') AS "value"`}},
			{{SQL: `, COALESCE(s.vartype, '') AS "type"`}},
			{{SQL: `, COALESCE(s.context, '') AS "context"`}},
			{
				{SQL: `, '' AS "access"`},
				{Min: v15, SQL: `, COALESCE(pg_catalog.array_to_string(p.paracl, E'\n'), '') AS "access"`},
			},
