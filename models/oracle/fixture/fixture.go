...
func ignoring(name, sqlstr string) Step {
	return at(name, `BEGIN EXECUTE IMMEDIATE '`+sqlstr+`'; EXCEPTION WHEN OTHERS THEN NULL; END;`)
}
...
var Everything = Fixture{
	...
	SELECT book_id, title FROM dbmeta_fixture.book WHERE published IS NOT NULL`),
		...
		{Name: "extras", Stmt: dbmeta.Stmt{{
			{SQL: `CREATE TABLE dbmeta_fixture.extras (
	extras_id NUMBER(10) PRIMARY KEY,
	label VARCHAR2(40),
	shouted VARCHAR2(80) GENERATED ALWAYS AS (UPPER(label))
)`},
			{Min: v12, SQL: `CREATE TABLE dbmeta_fixture.extras (
